...
const doshaConfig = {
  vata:  { border: "border-violet-400", bg: "bg-violet-50 dark:bg-violet-900/20", ring: "ring-violet-400", text: "text-violet-700 dark:text-violet-400", dot: "bg-violet-500", icon: "🌬️" },
  pitta: { border: "border-orange-400", bg: "bg-orange-50 dark:bg-orange-900/20", ring: "ring-orange-400", text: "text-orange-700 dark:text-orange-400", dot: "bg-orange-500", icon: "🔥" },
  kapha: { border: "border-teal-400",  bg: "bg-teal-50 dark:bg-teal-900/20",  ring: "ring-teal-400",  text: "text-teal-700 dark:text-teal-400",   dot: "bg-teal-500",   icon: "🌿" },
};

const QuizQuestion = ({ question, currentQuestionIndex, totalQuestions, selectedAnswer, onAnswerSelect, onNext, onPrev }) => {
      ...
      <div className="bg-white/90 dark:bg-white/5 backdrop-blur-xl border border-zinc-200 dark:border-white/10 rounded-2xl sm:rounded-3xl shadow-lg overflow-hidden transition-colors duration-300">
        ...
        <div className="px-5 sm:px-8 pt-6 sm:pt-8 pb-4 sm:pb-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-bold text-zinc-600 dark:text-stone-400 tabular-nums">
              ...
              <span className="text-zinc-400 dark:text-stone-600"> / {totalQuestions}</span>
            </span>
          </div>
        ...
        <div className="px-5 sm:px-8 pb-5 sm:pb-6">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-stone-100 leading-snug">
            ...
            <span className="text-amber-600 dark:text-amber-400">{question.question?.toLowerCase()}</span>?
          </h2>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm leading-relaxed transition-colors duration-200 ${isSelected && dosha ? `${dosha.text} font-medium` : "text-zinc-700 dark:text-stone-400"}`}>
                    {option.text}
                  </p>
                  {isSelected && dosha && (
                    <span className={`inline-block mt-1.5 text-xs font-semibold tracking-wide ${dosha.text}`}>
        ...
        <div className="px-5 sm:px-8 pb-6 sm:pb-8 mt-2 flex w-full items-center justify-between gap-3">
          ...
          >
            ...
            <span className="hidden xs:inline">{t('discover.quiz.previous', 'Previous')}</span>
          </Button>
          <Button
            onClick={onNext}
          ...
          >
            {isLast ? t('discover.quiz.see_result', 'See My Result') : t('discover.quiz.next_question', 'Next')}
